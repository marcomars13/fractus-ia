...
OUT_CSV = "results/batch_plonk_fractus_simple.csv"
SUMMARY_JSON = "results/summary_stats_simple.json"
...
def run_plonk_fractus(image_path):
    # Version brute (ancienne) : Plonk + simple correction Fractus
    # Ici tu mets juste ton algo Fractus simple, sans vote hybride
    h = (hash(Path(image_path).name) + 54321) % 10000
    return 40.0 + (h % 1000)/1000.0, 10.0 + ((h//1000) % 1000)/1000.0
