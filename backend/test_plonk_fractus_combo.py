...
def safe_run_plonk(img_path: str):
    """Lance Plonk en mode robuste, avec logs explicites."""
    try:
        ...
        if isinstance(out, dict) and "lat" in out and "lon" in out:
            print(f"🌍 Plonk OK pour {os.path.basename(img_path)} → lat={out['lat']:.4f}, lon={out['lon']:.4f}")
...
for _ in range(N_PAIRS):
    ...
    if plonk1 is not None and plonk2 is not None:
        ...
        dist_plonk = None
        combo_score = dist_fractus  # fallback Fractus seul

    results.append((combo_score, dist_fractus, dist_plonk, img1, img2))

# Trie
results.sort(key=lambda x: x[0])

print("\n🔝 Top 5 paires les plus proches (combo Skyline+Plonk) :")
for score, df, dp, i1, i2 in results[:5]:
    print(f"  {i1} vs {i2} → Combo={score:.3f} | Fractus={df:.3f} | Plonk={dp}")
